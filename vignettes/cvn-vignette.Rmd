...
especially through the concept of the *meta-graph*. 



The weight matrix is a $m \times m$ symmetric matrix. It reflects the (undirected)
*meta-graph* $W$, which encodes the smoothing structure between the graphs in a CVN. 
It must be set _a priori_.
...
Here are some examples how $W$ can look like: 
...
For this example, we will use a simulated data set where the variables can be thought of
to be z-score values of various biomarkers, each following a normal distribution. We 
are interested how the graphs differ by two external variables $U_1$ and $U_2$, 
...
Let us assume that in the example the external variables reflect 
_survey_ and _income level_ with
the three categories *baseline*, *first follow up*, *second follow up* for 'survey' 
and *low*, *middle* and *high* for 'income levels', respectively.

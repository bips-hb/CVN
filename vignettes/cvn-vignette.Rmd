...
*BMI* and *income level*. The biomarker values were transformed to normally distributed
z-score values. *BMI* has the three categories *underweight*, *normal weight*, 
*overweight* and *income levels* are *low*, *middle* and *high*.



### Data 

For this example, we will use a simulated data set where the variables in the graph
are z-score values of various biomarkers, each following a normal distribution. We 
are interested if the graphs differ by the two external variables *BMI* and *income level*, 
...
data$bmi <- sample(c("underweight","normal", "overweight"), n, replace = TRUE)
...
names(data_list) <- 
  apply(expand.grid(income = c("low","middle", "high"), 
                    bmi = c("underweight", "normal", "overweight")), 
        1,
...
Weight matrices can be plotted as a *meta-graph* or as *heatmap*:
```{r W-heat, fig.height=3.5}
par(mar = c(0, 0, 0, 0)) 

plot_weight_matrix(W_full, k = 3, l = 2)
...
In the following, however, we want to investigate the CVNs with ids 3 and 6.
This is easier when we extract this CVN into new single objects.
...
Sometimes is also helpful to explore the graph structures which will be shown in the 
next section. 
...
the number of edges within a graph as fixed. We output
therefore also the median number of edges and the interquartile range (IQR) 
across the $m$ estimated graphs of the CVN.
...
# Plot a CVN model
The function to plot the network structures of the CVN graphs requires the `visnet`package.
...
To explore the differences between the graphs, you can investigate
the number of edges in each CVN subgraph and analyze the Hamming distances 
...
```{r pp, eval=FALSE, message=FALSE}
plot_cvn <- plot(cvn, verbose = FALSE)
...
We have also proposed a method to interpolate a graph for which we do not have 
...
### wenn man das plotten möchte, sollte hier noch was programmiert werden
